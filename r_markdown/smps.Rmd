...
* save ultrafine for background analysis

```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  smps_ultrafine_bkgd <- dplyr::select(smps_merged, datetime, date, time, id,
                                       value, size, qc) %>%
                         dplyr::filter(size <= 100) %>%
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, results='markup', cache=FALSE}
  ...
  save(smps_ultrafine_bkgd, file = "../r_files/smps_ultrafine_bkgd.Rda")
```
